  ...
  switch (ctx.kind) {
    ...
    case LocationKind::BatchPoint:
      return "batch " + std::to_string(ctx.int_a) + " lane " +
             std::to_string(ctx.int_b) + " p=(" + std::to_string(ctx.point.x) +
             "," + std::to_string(ctx.point.y) + "," +
    ...
    if (!same_nonfinite) {
      ...
      if (result.first_mismatch.empty()) {
        result.first_mismatch =
            format_location(ctx) + " " + result.expected_name + "=" +
            std::to_string(expected) + " " + result.actual_name + "=" +
...
__m256 pack(LaneValues const& values) { return _mm256_loadu_ps(values.data()); }

void compare_vec3(Comparison& result, Vec3 expected, LaneValues const& xs,
                  LaneValues const& ys, LaneValues const& zs, int lane,
                  LocationContext ctx) {
  ctx.suffix = ".x";
...
Comparison compare_sdf(std::mt19937& generator,
                       render::RenderConfig const& config, std::string name) {
...
Comparison compare_sdf_packet_invariance(render::RenderConfig const& config) {
...
Comparison compare_normals(std::mt19937& generator,
  ...
  for (int batch = 0; batch < 64; ++batch) {
    ...
    for (int lane = 0; lane < kLanes; ++lane) {
      ...
      double dot =
          static_cast<double>(expected.x) * static_cast<double>(actual_vec.x) +
          static_cast<double>(expected.y) * static_cast<double>(actual_vec.y) +
          static_cast<double>(expected.z) * static_cast<double>(actual_vec.z);
      double cos_angle = dot / (len_exp * len_act);
      double angle =
          std::acos(std::clamp(cos_angle, -1.0, 1.0)) * radians_to_degrees;
...
RenderPair compare_render(Camera const& camera,
  ...
  for (ImageTask const& task : tasks) {
    ...
    wrote = stbi_write_png(path.string().c_str(), task.image->width(),
                           ...
                           task.image->data(),
                           static_cast<int>(task.image->row_stride())) != 0 &&
            wrote;
  }
  if (!wrote) {
...
int main(int argc, char* argv[]) {
  ...
  if (argc == 3) {
    ...
    if (parse_result.ec != std::errc{} ||
        parse_result.ptr != value.data() + value.size() || resolution <= 0 ||
        resolution > 4096 || resolution % kLanes != 0) {
  ...
  generator.seed(kDefaultSeed);
  results.push_back(compare_sdf(generator, ray_config, "scalar vs SIMD SDF"));
  generator.seed(kDefaultSeed);
  results.push_back(
      compare_sdf(generator, alternate_escape_config,
  ...
  RenderPair render_pair = compare_render(camera, ray_config, output_directory);
