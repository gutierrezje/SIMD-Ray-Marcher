...
void print_usage(char const* executable) {
  std::cout << "Usage: " << executable << " [--backend=scalar|simd|compare]\n";
}
...
DiffStats compare_images(render::Image const& scalar_image,
  ...
  for (std::size_t index = 0; index < scalar_image.size(); ++index) {
    ...
    int difference = scalar_value > simd_value ? scalar_value - simd_value
    ...
    stats.total_absolute_difference += static_cast<std::uint64_t>(difference);
  }
  return stats;
}
...
int run_single_backend(Backend backend, Camera const& camera) {
  ...
  std::cout << "Backend: " << (backend == Backend::scalar ? "scalar" : "simd")
            << '\n'
...
int run_comparison(Camera const& camera) {
  ...
  double scalar_elapsed =
      measure_seconds([&] { scalar::render(camera, kConfig, scalar_image); });
  double simd_elapsed =
      measure_seconds([&] { simd8::render(camera, kConfig, simd_image); });
  ...
  std::cout << "Scalar elapsed time: " << scalar_elapsed << " s\n"
            ...
            << "Mean absolute channel difference: " << mean_absolute_difference
            << '\n'
